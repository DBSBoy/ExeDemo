...
public class Stream_Sort_Filter {
    ...
    public static void println(Stream<Emp> stream) {
        stream.forEach(emp -> {
            System.out.printf("名字：%s，年纪：%s，薪水：%s%n", emp.getName(), emp.getAge(), emp.getSalary());
        });
    }
    ...
    public static void main(String[] args) {
        ...
        // 对list里的emp对象，取出薪水，并对薪水进行排序，然后输出薪水的内容，map操作，改变了Stream的泛型对象
        list.stream().map(Emp::getSalary).sorted().forEach(System.out::println);
        // 根据emp的属性name，进行排序,记住，sorted这些中间操作都是基于上一次的结果，这样就能实现两两比较
